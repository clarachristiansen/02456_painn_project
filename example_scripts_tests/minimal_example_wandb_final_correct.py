...
sweep_config = {
    ...
    'parameters': {
        'num_message_passing_layers': {'values': [1,2,3,4]}, # [2, 3, 4, 5]
        #'clip_value': {'values': [100,100]}
        'lr': {'values': [1e-5, 5e-4, 1e-4]}, # [1e-3, 5e-4, 1e-4]
        #'batch_size_train': {'values': [32, 64, 100]},
        'weight_decay': {'values': [0.1, 0.01, 0.001]} # [0.01, 0.001, 0.0001]
    }
}

sweep_id = wandb.sweep(sweep_config, project="Standard_noclampdistance_smoothing")
...
def main():
    ...
    with wandb.init(config=args.__dict__) as run:
        ...
        for epoch in pbar:
            ...
            # Early stopping
            if abs(smoothed_loss - best_val_loss_smooth) > 0.0001:
