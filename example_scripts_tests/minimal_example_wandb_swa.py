...
sweep_config = {
    ...
    'parameters': {
        'epoch_swa': {'values': [80, 100, 150]}, 
        'lr_swa': {'values': [0.0001, 0.0005, 0.001]},
    }
}

sweep_id = wandb.sweep(sweep_config, project="QM9_PaiNN_swa")
...
def main():
    ...
    with wandb.init(config=args.__dict__) as run:
        ...
        swa_scheduler = SWALR(optimizer, swa_lr=config.get("lr_swa", 0.05))
        ...
        for epoch in pbar:
            ...
            # SWA update
            if epoch > config.get("epoch_swa", 5):# * 0.75:  # Start SWA in the last 25% epochs
