...
sweep_config = {
    ...
    'parameters': {
        ...
        'lr': {'values': [1e-3, 5e-4, 1e-4]}, # [1e-3, 5e-4, 1e-4]
        #'batch_size_train': {'values': [32, 64, 100]},
        #'weight_decay': {'values': [0.01]} # [0.01, 0.001, 0.0001]
    }
}

sweep_id = wandb.sweep(sweep_config, project="QM9_PaiNN_wandb_lr_final")
    ...
    with wandb.init(config=args.__dict__) as run:
        ...
        for epoch in pbar:
            ...
            for batch in dm.train_dataloader():
                ...
                #else: 
                torch.nn.utils.clip_grad_value_(painn.parameters(), clip_value=config.get("clip_value", 1))
                optimizer.step()
